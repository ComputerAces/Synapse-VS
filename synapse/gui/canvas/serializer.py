                ...
                if node_class:
                    try:
                        ...
                        if "properties" in n_data:
                            # [MIGRATION] Merge properties carefully to preserve default options
                            # while restoring saved values.
                            logic.properties.update(n_data["properties"])
                        ...
                        # Check for SubGraph node with a graph path (Title Case or legacy lowercase)
    ...
    def deserialize_selection(self, data, offset=QPointF(30, 30)):
        ...
        if new_items and self.view:
            ...
            if not viewport_rect.intersects(pasted_rect):
                ...
                # Actually Wire items update their path based on port positions which update with nodes.
                # However, some wires might need an explicit update or be redrawn.
                # In Synapse, wires are in the scene and normally update via path logic.
                for item in self.scene.items():
