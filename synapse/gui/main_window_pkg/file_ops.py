        ...
        try:
            ...
            try:
                ...
                if was_modified:
                    ...
                    self.statusBar().showMessage(f"Graph migrated to v{data.get('version', '?')}", 5000)
                    
                    filename = os.path.basename(file_path)
                    res = QMessageBox.information(self, "Graph Migration", 
                        f"The graph '{filename}' was automatically patched to the latest version (v{data.get('version', '2.1.0')}).\n"
                        "Node properties have been standardized to 'Proper Case with Space'.\n\n"
                        "It is recommended to save the graph now to maintain this format.",
